...
isGrid = ENV['RUN_LOCAL'].to_s == 'true'
...
Before do |scenario|
  ...
  setup_local(driver) if isGrid
  setup_grid(driver) unless isGrid
...
After do |scenario|
  begin
    if scenario.failed?
      ...
      screenshot = "#{$VALUE}/test_screenshots/FAILED_#{scenario.name.tr(' ', '_').gsub(/[^0-9A-Za-z_]/, '')}_#{Time.new.localtime.strftime('%F %H.%M.%S%L')}.png"
